...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === "GET" && req.query?.public === "1" && id && id.length <= 120) {
      const publicColumns = "id,report_no,observation_id,date,time,location,department,observer_name,risk_level,category,status,observation_description,corrective_action,image1,image2,image3,image4,created_at";
      const publicResponse = await supabaseFetch(`/rest/v1/safety_reports?id=eq.${encodeURIComponent(id)}&status=neq.draft&select=${publicColumns}&limit=1`, { cache: "no-store" });
      const publicRows = await publicResponse.json().catch(() => []);
      if (!publicResponse.ok) return json(res, publicResponse.status, { error: "Unable to load public report" });
      if (!publicRows[0]) return json(res, 404, { error: "Report not found" });
      res.setHeader("Cache-Control", "private, no-store, max-age=0");
      return json(res, 200, toClient(publicRows[0]));
    }
