...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === "POST") {
      ...
      const refNo = `NCR-${year}-${Date.now().toString(36).toUpperCase()}`;
      const row = {
        ref_no: refNo,
        ...
        corrective_actions: Array.isArray(body.correctiveActions) ? body.correctiveActions : [],
      ...
      if (!response.ok) return json(res, response.status, { error: result?.message || "Unable to create NCR" });
