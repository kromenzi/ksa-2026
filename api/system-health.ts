...
const COLUMNS: Record<string, Set<string>> = {
  ...
  employee_violations:new Set(["id","ref_no","date","employee_name","employee_id","employee_record_id","department","occupation","position","factory","section","reference_to","violation","violation_description","notes","severity","status","data","supervisor_name","created_by","created_at","updated_at","escalation_id","escalated_at","escalated_by"]),
...
const DOCUMENT_STORAGE_BUCKET="board-uploads";
...
const HSE_IMAGE_SCOPES=new Set(["ncr","violation","monthly-task"]);
const isHseImageStoragePath=(value:string)=>/^hse-images\/(ncr|violation|reporting|monthly-task)\/[A-Za-z0-9_-]+\/[0-9]{4}-[0-9]{2}-[0-9]{2}\/[A-Za-z0-9._-]+$/.test(value);
