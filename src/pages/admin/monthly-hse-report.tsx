...
export default function MonthlyHseReportPage(){
 ...
 const patch=async(p:any)=>{if(!selected)return;const r=await apiRequest("PATCH",`/api/data?resource=monthly-hse-reports&id=${selected.id}`,p);const b=await r.json();if(!r.ok)return toast.error(b?.error||"Update failed");await refresh();};
