...
export default function NewNCR() {
  ...
  const createDepartment = async () => {
    ...
    try {
      ...
      setNewDepartment(""); setNewDepartmentCode(""); setShowAddDepartment(false);
      // Refresh the real Supabase-backed list, then select the new department.
  ...
  const submit = async () => {
    if (!form.department || !form.description.trim()) {
      toast.error(isAr ? "القسم ووصف عدم المطابقة مطلوبان" : "Department and description are required"); return;
    }
    if (!addNCR) return;
    setSaving(true);
    try {
      ...
      if (created?.id) navigate(`/admin/ncr/${created.id}`); else navigate("/admin/ncr");
    } catch (e:any) { toast.error(e?.message || (isAr ? "تعذر إنشاء NCR" : "Unable to create NCR")); }
    finally { setSaving(false); }
  };

  return (
    <main dir={isAr ? "rtl" : "ltr"} className="mx-auto w-full max-w-5xl space-y-6 pb-12">
        ...
        <Card dir={isAr ? "rtl" : "ltr"} className="overflow-hidden">
          ...
          <CardContent className="space-y-5">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div><div className="text-xs text-muted-foreground">{isAr ? "القسم" : "Department"}</div><div className="font-medium">{options.find((d:any)=>d.id===form.department)?.name || "—"}</div></div>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2"><Label>{isAr ? "القسم" : "Department"} <span className="text-destructive">*</span></Label>
                <Select value={form.department} onValueChange={v=>setField("department",v)}><SelectTrigger><SelectValue placeholder={isAr ? "اختر القسم" : "Select Department"} /></SelectTrigger><SelectContent>{options.map((d:any)=><SelectItem key={d.id} value={d.id}>{d.name} {d.code ? `(${d.code})` : ""}</SelectItem>)}</SelectContent></Select>
                ...
                {showAddDepartment && <div className="grid gap-2 rounded-md border p-3 sm:grid-cols-[1fr_180px_auto]"><Input value={newDepartment} onChange={e=>setNewDepartment(e.target.value)} placeholder={isAr ? "اسم القسم" : "Department name"} /><Input value={newDepartmentCode} onChange={e=>setNewDepartmentCode(e.target.value)} placeholder={isAr ? "الرمز (اختياري)" : "Code (optional)"} /><Button type="button" disabled={savingDepartment || !newDepartment.trim()} onClick={createDepartment}>{savingDepartment ? "..." : (isAr ? "إضافة" : "Add")}</Button></div>}
              </div>
              <div className="space-y-2"><Label>{isAr ? "الموقع" : "Location"}</Label><Input value={form.location} onChange={e=>setField("location",e.target.value)} placeholder={isAr ? "الموقع" : "Area / location"} /></div>
              <div className="space-y-2"><Label>{isAr ? "مستوى الخطورة" : "Severity"}</Label><Select value={form.severity} onValueChange={v=>setField("severity",v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="low">Low / منخفض</SelectItem><SelectItem value="medium">Medium / متوسط</SelectItem><SelectItem value="high">High / عالي</SelectItem><SelectItem value="critical">Critical / حرج</SelectItem></SelectContent></Select></div>
            </div>
            <div className="space-y-2"><Label>{isAr ? "وصف عدم المطابقة" : "Non-Conformity Description"} <span className="text-destructive">*</span></Label><Textarea rows={5} value={form.description} onChange={e=>setField("description",e.target.value)} /></div>
            <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div className="flex justify-end gap-2 border-t pt-4"><Button type="button" variant="outline" onClick={()=>navigate("/admin/ncr")}>{isAr ? "إلغاء" : "Cancel"}</Button><Button type="button" onClick={submit} disabled={saving}>{saving ? (isAr ? "جاري الحفظ..." : "Saving...") : (isAr ? "إنشاء NCR" : "Create NCR")}</Button></div>
          </CardContent>
        </Card>
      )}
    </main>
