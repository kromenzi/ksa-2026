...
const printCss = (rtl: boolean) => `
  ...
  * { box-sizing: border-box; forced-color-adjust: none !important; }
  ...
  .incident-print-report { display: block !important; width: 100%; direction: ${rtl ? "rtl" : "ltr"}; text-align: ${rtl ? "right" : "left"}; background-color: #ffffff !important; color: #111827 !important; color-scheme: light !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
  ...
  @media print { html, body { background: #ffffff !important; color: #111827 !important; color-scheme: light !important; } }
`;
...
export default function AdminIncidentsPage() {
  ...
  const openEscalation = (refNo: string) => {
    // Keep the incident page in place. Open the existing escalation dashboard in a new tab.
    const url = `/admin/escalations/dashboard?source=${encodeURIComponent(refNo)}`;
    const opened = window.open(url, "_blank", "noopener,noreferrer");
    if (!opened) setEscalationRef(refNo);
  };
  ...
  const printIncident = () => {
    ...
    if (!source) return;
    const popup = window.open("", "incident-print", "width=960,height=1200");
    if (!popup) return;
    ...
    popup.document.open();
    popup.document.write(`<!doctype html><html lang="${isAr ? "ar" : "en"}" dir="${isAr ? "rtl" : "ltr"}><head><meta charset="utf-8"/><meta name="color-scheme" content="light"/><meta name="supported-color-schemes" content="light"/><title>${esc(activeIncident.refNo)} - Incident / Near Miss</title><style>${printCss(isAr)}</style></head><body>${clone.outerHTML}<script>window.onload=function(){setTimeout(function(){window.focus();window.print();},300)};<\\/script></body></html>`);
  ...
  return (
    <div className="space-y-6" data-testid="admin-incidents-page">
      <style>{`@media screen{.incident-print-report{display:none!important}}@media print{html,body{background:#fff!important;color:#111827!important;color-scheme:light!important}.incident-print-report{display:block!important;forced-color-adjust:none!important}.incident-screen-ui{display:none!important}}`}</style>
      ...
      {escalationRef && <Dialog open={!!escalationRef} onOpenChange={(open) => !open && setEscalationRef(null)}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>{isAr ? "التصعيد" : "Escalation"}</DialogTitle></DialogHeader><div className="rounded-lg border bg-muted/30 p-4 text-sm">{isAr ? `تعذر فتح لوحة التصعيد في تبويب جديد. رقم الحادث: ${escalationRef}` : `The escalation dashboard could not be opened in a new tab. Incident: ${escalationRef}`}</div><DialogFooter><Button type="button" variant="outline" onClick={() => setEscalationRef(null)}>{isAr ? "إغلاق" : "Close"}</Button><Button type="button" onClick={() => window.open(`/admin/escalations/dashboard?source=${encodeURIComponent(escalationRef)}`, "_blank")}>{isAr ? "فتح لوحة التصعيد" : "Open Escalation Dashboard"}</Button></DialogFooter></DialogContent></Dialog>}
