...
export default function AdminIncidentsPage() {
  ...
  const printIncident = () => {
    ...
    popup.document.open();
    popup.document.write(`<!doctype html><html lang="${isAr ? "ar" : "en"}" dir="${isAr ? "rtl" : "ltr"}><head><meta charset="utf-8"/><meta name="color-scheme" content="light"/><meta name="supported-color-schemes" content="light"/><title>${esc(activeIncident.refNo)} - Incident / Near Miss</title><style>${printCss(isAr)}</style></head><body>${clone.outerHTML}</body></html>`);
    popup.document.close();
    setTimeout(() => { popup.focus(); popup.print(); }, 700);
  };
