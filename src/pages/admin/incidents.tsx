...
const printCss = (rtl: boolean) => `
  ...
  * { box-sizing: border-box; }
  ...
  body { font-family: Arial, Helvetica, sans-serif; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; color-adjust: exact !important; }
  .incident-print-report { display: block !important; width: 100%; direction: ${rtl ? "rtl" : "ltr"}; text-align: ${rtl ? "right" : "left"}; background: #fff !important; color: #111827 !important; }
  .incident-print-page { width: 100%; }
  .incident-print-header { border: 2px solid #b91c1c; border-radius: 10px; overflow: hidden; margin-bottom: 12px; }
  .incident-print-header-title { color: #fff !important; background: #b91c1c !important; padding: 12px 14px; font-size: 18px; font-weight: 800; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; box-shadow: inset 0 0 0 1000px #b91c1c; }
  .incident-print-header-subtitle { color: #7f1d1d !important; background: #fef2f2 !important; padding: 7px 14px; font-size: 10px; box-shadow: inset 0 0 0 1000px #fef2f2; }
  ...
  .incident-print-section-title { color: #fff !important; background: #1f2937 !important; box-shadow: inset 0 0 0 1000px #1f2937; padding: 7px 9px; border-radius: 5px; font-size: 11px; font-weight: 800; margin-bottom: 7px; }
  .incident-print-why { background: #eff6ff !important; box-shadow: inset 0 0 0 1000px #eff6ff; border: 1px solid #bfdbfe; border-radius: 6px; padding: 7px; margin-bottom: 6px; }
  .incident-print-why strong { color: #1d4ed8 !important; }
  .incident-print-fishbone { background: #f5f3ff !important; box-shadow: inset 0 0 0 1000px #f5f3ff; border: 1px solid #ddd6fe; border-radius: 6px; padding: 7px; margin-bottom: 6px; }
  .incident-print-table { width: 100%; border-collapse: collapse; table-layout: fixed; }
  .incident-print-table th, .incident-print-table td { border: 1px solid #d1d5db; padding: 7px 8px; font-size: 10px; line-height: 1.45; vertical-align: top; }
  ...
  .incident-print-badge { display: inline-block; padding: 3px 8px; border-radius: 999px; font-size: 9px; font-weight: 700; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
...
export default function AdminIncidentsPage() {
  ...
  const openEscalation = (refNo: string) => {
    window.location.assign(`/admin/escalations/dashboard?source=${encodeURIComponent(refNo)}`);
  };

  const printIncident = () => {
    if (!activeIncident) return;
    const source = document.querySelector<HTMLElement>(".incident-print-report");
    if (!source) return;

    const popup = window.open("", "incident-print", "width=960,height=1200,noopener,noreferrer");
    if (!popup) return;
    ...
    const html = `<!doctype html><html lang="${isAr ? "ar" : "en"}" dir="${isAr ? "rtl" : "ltr"}"><head><meta charset="utf-8"/><meta name="color-scheme" content="light"/><title>${esc(activeIncident.refNo)} - Incident / Near Miss</title><style>${printCss(isAr)}</style></head><body>${clone.outerHTML}<script>window.onload=function(){setTimeout(function(){window.focus();window.print();},250)};<\/script></body></html>`;
  ...
  return (
    <div className="space-y-6" data-testid="admin-incidents-page">
      <style>{`@media screen{.incident-print-report{display:none!important}}@media print{body{background:#fff!important}.incident-print-report{display:block!important}.incident-screen-ui{display:none!important}}`}</style>
      ...
      <div className="incident-screen-ui space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <Button onClick={createIncident} className="gap-2 bg-red-600 hover:bg-red-700"><Plus className="h-4 w-4" />{isAr ? "تسجيل حادث / وشيكة جديد" : "Report Incident / Near Miss"}</Button>
        </div>
