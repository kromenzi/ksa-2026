...
export default function EscalationMatrix() {
  ...
  const [rules, setRules] = useState<MatrixRule[]>(INITIAL_RULES);
  ...
  const openNew = () => { setMode("new"); setIndex(null); setSeverity("HIGH"); setTimeline("24 Hours"); setLevel("Level 3"); setRole("HSE Manager / HSE Lead"); setAutoEscalate("true"); setOpen(true); };
  const openEdit = (rule: MatrixRule, i: number) => { setMode("edit"); setIndex(i); setSeverity(rule.severity); setTimeline(rule.timeline); setLevel(rule.level); setRole(rule.role); setAutoEscalate(String(rule.autoEscalate)); setOpen(true); };
  const saveRule = () => {
    const next: MatrixRule = { severity, timeline, level, role: role.trim() || (isAr ? "دور مسؤول" : "Responsible Role"), autoEscalate: autoEscalate === "true" };
    setRules(prev => mode === "edit" && index !== null ? prev.map((r, i) => i === index ? next : r) : [...prev, next]);
    setOpen(false);
  };
