...
export default function AdminEmergencyPage(){
  ...
  const [selectedDrill,setSelectedDrill]=useState<DrillRecord|null>(null); const [deletingId,setDeletingId]=useState<string|null>(null); const [isPrintOpen,setIsPrintOpen]=useState(false); const [printItem,setPrintItem]=useState<any>(null);
  const {data:rows=[],isLoading}=useQuery<EmergencyApiRow[]>({queryKey:["/api/emergency"],queryFn:async()=>{const r=await apiRequest("GET","/api/emergency");const p=await r.json();return Array.isArray(p)?p:[];},staleTime:0});
  const drills=useMemo(()=>rows.map(toDrill),[rows]);
  const refresh=async()=>{await queryClient.invalidateQueries({queryKey:["/api/emergency"]});await queryClient.refetchQueries({queryKey:["/api/emergency"],type:"active"});};
