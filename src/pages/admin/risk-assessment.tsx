...
export default function AdminRiskAssessmentPage() {
  ...
  const handlePrintRiskTableAsImage = async () => {
    ...
    const riskLabel = (score: number) => score >= 15
      ? (isAr ? "حرج / مرتفع" : "High / Critical")
      : score >= 8
        ? (isAr ? "متوسط" : "Medium")
        : (isAr ? "منخفض" : "Low");
    const statusLabel = (status: RiskItem["status"]) => !isAr
      ? status
    ...
    const rowsHtml = rows.map((item) => `
      <tr>
        ...
        <td><strong>${esc(item.activity)}</strong><div class="muted">${esc(item.hazard)}</div></td>
        <td><strong>${esc(item.location)}</strong><div class="muted">${esc(item.department)}</div></td>
        <td>${item.initialRiskScore} - ${riskLabel(item.initialRiskScore)}<div class="muted">L${item.initialLikelihood} × S${item.initialSeverity}</div></td>
        <td>${item.residualRiskScore} - ${riskLabel(item.residualRiskScore)}<div class="muted">L${item.residualLikelihood} × S${item.residualSeverity}</div></td>
        <td>${esc(statusLabel(item.status))}<div class="muted">${esc(item.reviewDate)}</div></td>
      </tr>`).join('');
    ...
    const html = `<div xmlns="http://www.w3.org/1999/xhtml" dir="${isAr ? 'rtl' : 'ltr'}" style="font-family:Arial,sans-serif;color:#111827;background:#fff;padding:50px;box-sizing:border-box;width:100%;height:100%">
      ...
      <table style="width:100%;border-collapse:collapse;table-layout:fixed;font-size:18px">
        <thead><tr>${headers.map((h) => `<th style="background:#e2e8f0;border:1px solid #94a3b8;padding:14px">${esc(h)}</th>`).join('')}</tr></thead>
        <tbody>${rowsHtml || `<tr><td colspan="6" style="padding:24px;text-align:center;border:1px solid #cbd5e1">${isAr ? 'لا توجد سجلات' : 'No records'}</td></tr>`}</tbody>
      </table>
      <div style="margin-top:16px;color:#475569">${isAr ? `إجمالي السجلات: ${rows.length}` : `Total records: ${rows.length}`}</div>
    </div>`;

    const width = 2400;
    const height = Math.max(1000, 360 + Math.max(rows.length, 1) * 190);
    ...
    img.onload = () => {
      ...
      if (!win) {
        toast({ title: isAr ? 'تعذر فتح نافذة الطباعة' : 'Unable to open print window', variant: 'destructive' });
        return;
      }

      win.document.write(`<!doctype html><html><head><title>${esc(title)}</title><style>@page{size:A4 landscape;margin:8mm}html,body{margin:0;background:#fff}body{display:flex;justify-content:center}img{width:100%;height:auto;display:block}</style></head><body><img src="${png}" alt="${esc(title)}"></body></html>`);
      win.document.close();
    ...
    <div className="space-y-6" data-testid="admin-risk-assessment-page">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 text-white p-6 rounded-2xl shadow-xl border border-slate-800">
        ...
        <div className="flex items-center gap-2">
          <Button onClick={handlePrintRiskTableAsImage} variant="outline" className="gap-2 bg-white border-slate-300 text-slate-800 hover:bg-slate-100" data-testid="button-print-risk-table-image">
